The filesystem-backed implementation of the data-source contract at [packages/server/src/source/fs.ts](packages/server/src/source/fs.ts). Ships two classes — `FsArchitectureDataSource` (reads `manifest.json`, `diagrams/*.json`, `descriptions/*.md`) and `FsLayoutStore` (reads and atomically writes `.tecture/layouts/*.json`). The CLI constructs both and injects them into `createApp`; downstream consumers can ignore this file entirely and ship their own implementation.
...
- Implement `LayoutStore.loadLayout / saveLayout` over a configurable `.tecture/` root, using a temp-file + `rename` write to keep saves atomic.
