...
export interface WebDataSource {
  ...
  /**
   * Load the drift report generated by the architecture-docs skill's evidence
   * script, or null when none exists. Optional: hosts without drift support
   * simply omit it and the viewer shows no drift UI.
   */
...
export function createHttpDataSource(baseUrl = ""): WebDataSource {
  ...
  return {
    ...
    loadDrift: () =>
      fetchJson<DriftReport | null>(`${base}/api/architecture/drift`).catch(
        () => null,
      ),
