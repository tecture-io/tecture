...
/**
 * Reads the drift report generated by the architecture-docs skill's
 * evidence script (`<tectureRoot>/drift.json`). Tolerant like FsLayoutStore:
 * a missing or malformed file is a normal state, never an error — the viewer
 * simply shows no drift.
 */
export class FsDriftReader {
  constructor(private readonly tectureRoot: string) {}

  async load(): Promise<DriftReport | null> {
    const path = safeJoin(this.tectureRoot, "drift.json");
    let raw: string;
    try {
      raw = await readFile(path, "utf8");
    } catch (err) {
      if (isNodeErrnoError(err) && err.code === "ENOENT") return null;
      console.warn(`[tecture] failed to read drift report ${path}: ${String(err)}`);
      return null;
    }
    ...
    try {
      ...
      console.warn(`[tecture] malformed drift JSON at ${path}: ${String(err)}`);
      return null;
    }

    const report = parseDriftReport(parsed);
    if (!report) {
      console.warn(`[tecture] unusable drift report at ${path} (wrong shape or version)`);
    }
    return report;
  }
}
