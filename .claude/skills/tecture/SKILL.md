...
Only first-party code nodes are worth a deep-dive — `service`/`frontend`/`gateway`/component nodes, or anything with a `path`. There's nothing to investigate in a person, an external SaaS, or managed infra; if asked to deep-dive one, say so. The main agent is the **sole writer** (sub-agents return text, never edit files), and a deep-dive is **prose-only** — it never changes diagram structure; a suspected missing node/edge is reported to the user, not applied. When a deep-dive finds a component complex enough to warrant its own drill-down diagram, it **suggests** creating one (a new sub-diagram via `subDiagramId`) and offers to author it on request — again, suggested, not applied.
