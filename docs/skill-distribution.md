...
3. **Sync the mirror** — copy the updated skill tree into `tecture-io/architecture-docs`, commit, push. `gh skill` and manual clone auto-pick up the latest commit; no further action needed for those channels.
...
- **[skills.sh](https://skills.sh) (`npx skills add`)** — dropped. Installs to the canonical `~/.agents/skills/` without creating the `~/.claude/skills/` symlink, so the skill is invisible to Claude Code, our primary target ([vercel-labs/skills#744](https://github.com/vercel-labs/skills/issues/744)). Superseded by `@tecture/skill`, which copies into each agent's native directory.
